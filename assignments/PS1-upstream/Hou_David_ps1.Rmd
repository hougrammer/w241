...
**$E[Y_{i}(1)|d_{i}=1]$ is the expectation of the potential outcome given one specifc assignment of treatments.  $E[Y_{i}(1)|D_{i}=1]$ is the expectation over all possible assignments of treatments.  For the example in the book, $E[Y_{i}(1)|d_{i}=1]$ is the expected potential outcome when villages 1 and 7 are chosen for treatment.  $E[Y_{i}(1)|D_{i}=1]$ is the expected potential outcome over all possible village partitions.**
...
$$
\begin{aligned}
E[Y_{i}(0)] &= \frac{5+3+10+5+10}{5} &= 6.6 \\
E[Y_{i}(1)] &= \frac{6+8+12+5+8}{5} &= 7.8 \\
E[Y_{i}(1) - Y_{i}(0)] &= \frac{1+5+2+0-2}{5} &= 1.2 \\
...
|20             | n: 1    | n: 0    | n: 1      | 0.29         |
|               | %: 0.14 | %: 0.00 | %: 0.14   |              |
| Marginal $Y_{i}(1)$     | 0.57    | 0.14      | 0.29   | 1.0 |
...
library(data.table)
dt = data.table(d)
answer.POe <- mean(dt[child %% 2 == 1, y1] - dt[child %% 2 == 0, y0])
answer.POe
``` 
...
$$
\sum_{i=1}^{N-1}{ N \choose i}
$$

```{r}
answer.POg <- sum(choose(10, 1:9))
answer.POg
```
...
```{r}
answer.POh <- mean(dt[child <= 5, y1] - dt[child >= 6, y0])
answer.POh
``` 

i. Compare your answer in (h) to the true ATE.  Intuitively, what causes the difference?

**The children who who chose to play an average of more than 10 hours might do so because they like to play outside, and vice versa.  One could envision that children whose eyesights are damaged by being outside might choose to stay inside.  The converse is probably true as well.  Therefore we see much larger effect here.**
...
**The prisoners who were already reading 3 hours per day may have some other reason to be less violently inclined.  This selection bias causes ${E[Y_{i}(0)|D_{i}=0] \ne E[Y_{i}(0)|D_{i}=1]}$ and $E{[Y_{i}(1)|D_{i}=0] \ne E[Y_{i}(1)|D_{i}=1]}$.  These equalities represent the potential outcomes for prisoners in the reading and non-reading group.  If the prisoners had been truly randomly assigned to the treatment and control, the potential outcomes would be same regardless of whether they were in treatment or control.  That is, the prisoners in treatment group would have the same reduction of violence from reading as the ones in the control group.  However, in this case, some prisoners self-selected to be in the treatment group.  It is likely that these prisoners naturally have a low potential outcome of violent encounters.**
