...
```
**Both methods predict a CACE of `r round(cace, 4)`.  The 2-stage least squares estimates a standard error of `r round(co[2,2],4)`.  The p-value is very low, so it does seem like `contact` had an effect.**
...
**The estimated CACE is `r round(co[2,1], 4)` (`r round(co[2,2], 4)`).  The estimate is insignificant, which is consistent with the assumption that the placebo has no effect on turnout.**
...
(co = coeftest(m))
```
**The estimated CACE is `r round(co[2,1], 4)` (`r round(co[2,2], 4)`).  It is much smaller compared to part d.  This could suggest that there may be some placebo effect on Y.  Although part c showed no significant effect, the estimate was still positive.**
...
(co = coeftest(m))
```
<!--What an absurd error.  Apparently PDF knit breaks if R tries to output scientific notation.  The format() here is to prevent that. -->
**The estimated effect is `r format(co[2,1])` (`r format(co[2,2])`).**
...
(co = coeftest(m))
```
**The estimated effect is `r format(co[2,1])` (`r format(co[2,2])`).  This shows no evidence of anticipation.  Whether she runs the next day has no effect on the Tetris score in the previous day.**
...
(co = coeftest(m))
```
**The estimated effect is `r format(co[2,1])` (`r format(co[2,2])`).  The estimate is statistically significant.**
...
(co = coeftest(m))
```
**The estimated effect is `r format(co[2,1])` (`r format(co[2,2])`).  Energy is not significantly affected by running.**

```{r}
m = d[, lm(gre ~ run)]
(co = coeftest(m))
```
**The estimated effect is `r format(co[2,1])` (`r format(co[2,2])`).  GRE scores are not significantly affected by running.**
